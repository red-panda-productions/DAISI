...
TEST(RecorderTests, WriteDecisions)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        expectedDecisionsData << bits(timestamp) << bits(decisionTuple.GetSteerAmount()) << bits(decisionTuple.GetAccelAmount())
                              << bits(decisionTuple.GetBrakeAmount()) << bits(static_cast<float>(decisionTuple.GetGearAmount()));
