...
Recorder::Recorder(const std::string& p_dirName, const std::string& p_fileNamePattern, int p_userParamAmount, int p_decisionParamAmount) : m_userParamAmount(p_userParamAmount), m_decisionParamAmount(p_decisionParamAmount)
{
    ...
    std::experimental::filesystem::path path = std::experimental::filesystem::path(recordingsFolder).append(USER_INPUT_RECORDING_FILE_NAME);
...
void Recorder::WriteDecisions(const float* p_input, const unsigned long p_currentTime) {
    WriteRecording(p_input, p_currentTime, m_decisionsRecordingFile, m_decisionParamAmount, false, nullptr);
}
...
void Recorder::WriteRecording(const float* p_input, const double p_currentTime, std::ofstream& p_file, const int p_paramAmount, bool p_useCompression, float* p_prevInput)
{
