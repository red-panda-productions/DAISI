/***************************************************************************
    ...
    email                : torcs@free.fr
    version              : $Id: track1.cpp 6752 2019-11-05 14:03:20Z beaglejoe $
...
AddSide(tTrackSeg *curSeg, tdble width, char *material, int side, int type, void *TrackHandle)
{
    ...
    snprintf(path, sizeof(path), "%s/%s/%s", TRK_SECT_SURFACES, TRK_LST_SURF, material);
...
ReadTrack1(tTrack *theTrack, void *TrackHandle, tRoadCam **camList)
{
	...
	//snprintf(path, sizeof(path), "%s/%s/%s", TRK_SECT_CAM, TRK_LST_CAM, curSeg->name);
	curSeg->id = curindex;
	curSeg->width = width;
