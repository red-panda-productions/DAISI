...
void SDAHud::loadDefaults(const tCarElt* curCar)
{
...
void SDAHud::selectBoard(int val)
{
    char path[1024];
    snprintf(path, sizeof(path), "%s/%d", GR_SCT_DISPMODE, id);
 ...
 */
void SDAHud::grDispDebug(const tSituation* s, const cGrFrameInfo* frame)
{
    char buf[BUFSIZE];
...
void SDAHud::grDrawGauge(tdble X1, tdble Y1, tdble H,
...
void SDAHud::grDispSplitScreenIndicator()
{
...
void SDAHud::grDispCounterBoard2()
{
    // RPM
...
void SDAHud::refreshBoard(tSituation* s, const cGrFrameInfo* frameInfo,
...
void SDAHud::grDispIntervention()
{
