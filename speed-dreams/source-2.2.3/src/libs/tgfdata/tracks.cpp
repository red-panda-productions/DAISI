/***************************************************************************
    ...
    version              : $Id: tracks.cpp 7011 2020-04-30 15:23:52Z iobyte $
                                       ...
                                       int nSearchDir, bool bSkipFrom) const
{
    ...
    // Check category.
    if (!strCatId.empty() && std::find(_pPrivate->vecCatIds.begin(), _pPrivate->vecCatIds.end(), strCatId) == _pPrivate->vecCatIds.end())
    {
...
GfTrack::GfTrack()
    : _fLength(-1), _fWidth(-1), _nMaxPitSlots(-1), _bUsable(false)
{
}
